/* NDHCE Exam Simulator, Application Logic */

const ACCESS_CODE  = "NDHCE9000";
const EXAM_SECONDS = 14400;
const PASSING_PCT  = 75;
const STORAGE_KEY  = "ndhce_exam_state_v1";
const SIM_Q_COUNT  = 200;  // simulator serves this project's configured pool size (config.json exam.sim_questions)
const DOMAIN_LABELS = {"clinical_therapy": "Clinical Therapy", "prevention_education_and_health_promotion": "Prevention Education and Health Promotion", "collaboration": "Collaboration", "professionalism": "Professionalism", "practice_management": "Practice Management", "evidence_informed_practice": "Evidence Informed Practice", "communication": "Communication"};  // maps domain key -> human-readable label for display
...
window.addEventListener("DOMContentLoaded", () => {
  questions = pickQuestions((window.EXAM_QUESTIONS || []).slice(), SIM_Q_COUNT);
  restoreState();
...
// Shuffle by UNIT, never by individual question. An NDHCE case cluster is 4 to
// 6 questions sharing one client scenario, and they must stay together and in
// their authored order: questions later in a cluster refer back to the same
// client. Shuffling every question individually scatters the clusters across
// the exam, which on CNPLE only showed up when the live site was driven with a
// real browser. Truncation is also done on a unit boundary so a cluster can
// never be cut in half.
function pickQuestions(all, limit) {
  const units = [];
  const byCase = new Map();
  for (const q of all) {
    if (!q.case_id) { units.push([q]); continue; }
    if (!byCase.has(q.case_id)) {
      const u = [];
      byCase.set(q.case_id, u);
      units.push(u);
    }
    byCase.get(q.case_id).push(q);
  }
  shuffleUnits(units);
...
function breakAnswerRuns(arr) {
  const free = i => arr[i] && !arr[i].case_id;
...
// Case-based NDHCE items share one client scenario across 4 to 6 linked
// questions. Show it above every question in the cluster so a candidate never
// has to page backwards to reread the case.
function renderCase(q) {
  const wrap = document.getElementById("q-case-wrap");
  if (!wrap) return;
  if (!q.case_text) {
    wrap.innerHTML = "";
  ...
  const body = String(q.case_text).split("\n")
    .filter(line => line.trim())
    .map(line => `<p>${escapeHTML(line.trim())}</p>`).join("");
  wrap.innerHTML = `<div class="case-label">Case ${escapeHTML(q.case_id)}</div>${body}`;
